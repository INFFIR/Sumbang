...
const ManageContent = () => {
  ...
  const handleSubmit = async () => {
    if (!formData.title && !formData.description && !formData.media && modalType === "content") return alert("Minimal satu field harus diisi.");
    if (!formData.title && !formData.description && modalType === "service") return alert("Minimal satu field harus diisi.");

    const form = new FormData();
    form.append("title", formData.title);
    form.append("description", formData.description);
    if (formData.media && modalType === "content") form.append("media", formData.media);

    try {
      if (editData) {
        await axios.post(`${process.env.REACT_APP_API_URL}/api/manage/${modalType}/edit/${editData.id}`, form, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } else {
        await axios.post(`${process.env.REACT_APP_API_URL}/api/manage/${modalType}/create`, form, {
          headers: { Authorization: `Bearer ${token}` },
        });
      }
      handleClose();
            ...
            <tbody>
              {contentList.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>{item.media ? <a href={`data:image/jpeg;base64,${item.media}`} target="_blank" rel="noreferrer">Lihat</a> : "-"}</td>
                  <td>
